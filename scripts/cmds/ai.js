...
const services = [
  { url: 'http://us-01.xeh.sh:25878/chatgpt', param: { prompt: 'prompt' }, isCustom: true }
];
...
async function callService(service, prompt, senderID) {
  if (service.isCustom) {
    try {
      ...
      return response.data.gpt || response.data;  // Adjust based on your API's response format
    } catch (error) {
